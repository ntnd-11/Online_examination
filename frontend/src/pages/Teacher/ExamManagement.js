...
function ExamManagement() {
    ...
    const handleDetailClick = (id) => {
        navigate(`/exam-management/${id}`);
    };

    const handleDeleteClick = async (id) => {
        setIsLoading(true);
        TeacherService.removeExamById(id)
            .then((response) => {
                setIsLoading(false);
    ...
    useEffect(() => {
        TeacherService.getAllExams(page, perPage)
            .then((response) => {
                if (response.status === ApiStatus.SUCCESS) {
                    let countPages;
    ...
    return (
        <Box bg={'white'} mr={4} borderRadius={5} pt={4} pb={4} pl={4} pr={4} overflow={'auto'}>
            <Modal isOpen={isOpen} onClose={onClose}>
                <ModalOverlay />
                <ModalContent>
                    <ModalHeader>Xóa bài thi</ModalHeader>
                    <ModalCloseButton />
                    <ModalBody>
                        <Text>Bạn có chắc chắn muốn xóa bài thi này?</Text>
                    </ModalBody>
                    ...
                    <ModalFooter>
                        <Button
                            ...
                            onClick={(e) => {
                                e.preventDefault();
                                handleDeleteClick(selectedExamId);
                            }}
                        >
                            Xác nhận
                        </Button>
            ...
            <Box marginBottom={4}>
                ...
                <Box display={'flex'}>
                    ...
                    >
                        <Icon mr={2} as={FaPlus} />
                        <Text>Thêm bài thi</Text>
                    </Link>
                </Box>
                <Box mt={4}>
                    <Text>Số lượng bài thi trong 1 trang</Text>
                ...
                <Box overflow={'auto'}>
                    <TableContainer fontSize={12}>
                        <Table>
                            ...
                            <Tbody>
                                {exams.length <= 0 ? (
                                    <Td textAlign={'center'} fontSize={14} fontWeight={'500'} colSpan={7}>
                                        <Text>Chưa có bài thi</Text>
                                    </Td>
                                ) : (
